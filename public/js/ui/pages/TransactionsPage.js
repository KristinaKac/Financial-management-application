...
class TransactionsPage {
  ...
  constructor(element) {
    if (!element) {
      throw new Error('Был передан пустой элемент');
    }
  ...
  registerEvents() {
    ...
    this.element.addEventListener('click', (e) => {
      if (e.target.closest('.transaction__remove')) {
        const el = e.target.closest('.transaction__remove');
  ...
  formatDate(date) {
    ...
    const formatedDate = d.toLocaleDateString('ru-RU', { year: "numeric", month: "long", day: "numeric" });
    const formatedTime = d.toLocaleTimeString('ru-RU', { hour: "numeric", minute: "numeric" });
    return `${formatedDate} в ${formatedTime}`;
  }
  ...
  renderTransactions(data) {
    ...
    content.innerHTML = '';
    data.forEach(el => content.insertAdjacentHTML('beforeend', this.getTransactionHTML(el)));
  }
}